...
#define BUFFER_SIZE BUFFER_SIZE_DPU*64
#define mallocSize BUFFER_SIZE*sizeof(uint32_t)
...
uint32_t key[16] = {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};  
int8_t chiave[16] = {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};  
uint32_t RC[] = {0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1B, 0x36};
uint32_t EK[176];
...
void initBuffer(uint32_t* b, const int size)
{
	for(int i=0;i<size;++i)
	{
		b[i] = (rand()%26 + 'A') && 0x000000FF;
	}
}

void keyExpand(uint32_t* wb, const uint32_t* k){
	int i=0;
	for(i=0;i<44*4;++i)
	{
		if(i<16) { wb[i]=k[i]; }
...
int main()
{
	...
	while(DPU_dpubuffers < tot_DPU_buffer)
	{
		...
		DPU_RANK_FOREACH(setRanks,rank,each_rank)
		{
			...
			{
				uint32_t* bufferDPU = bufferHost + offDPU;
				DPU_ASSERT(dpu_prepare_xfer(dpu,bufferDPU));	//Prepare
				offDPU += BUFFER_SIZE_DPU;
				if(offDPU==BUFFER_SIZE_DPU*64){ offDPU = 0; }
				DPU_dpubuffers++;
			} //end FOR_EACH
			DPU_ASSERT(dpu_push_xfer(rank,DPU_XFER_TO_DPU,"buffer",0,BUFFER_SIZE_DPU*sizeof(uint32_t),DPU_XFER_ASYNC));	//Transfer	
		...
		DPU_RANK_FOREACH(setRanks,rank,each_rank)
		{
			DPU_FOREACH(rank,dpu,each_dpu)
			{
				...
				offDPUCrypted=(offDPUCrypted+BUFFER_SIZE_DPU)%(BUFFER_SIZE_DPU*64);
			}
			DPU_ASSERT(dpu_push_xfer(rank,DPU_XFER_FROM_DPU,"crypted",0,BUFFER_SIZE_DPU*sizeof(uint32_t),DPU_XFER_ASYNC));
		}
	}
	dpu_sync(setRanks);
	...
	while(NI_dpubuffers < tot_DPU_buffer)
	{
		for(int i=0;i<nrRANKS;++i) //FOREACH_RANK
		{
			for(int j=0;j<nrDPUs_perRANK[i];++j) //FOREACH
			{
				...
				{
					aes128_enc((char*)bufferHost+offHOST,(char*)cryptedNI+offHOST);
					offHOST=(offHOST+16)%(BUFFER_SIZE_DPU*64);
				}
				NI_dpubuffers++;
			}
		}
